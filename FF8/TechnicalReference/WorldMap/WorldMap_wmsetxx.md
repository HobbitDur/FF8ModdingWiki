...
| −247 | `0xFF09` | `CHECK_VEHICLE_TYPE`          | Current vehicle equals `param`. Known values: 33 = bike, **48 = the mobile Balamb Garden**, 49 = the two big ships, **50 = the Ragnarok**, 128 = on foot, 129 = the walking party, 130 = Galbadia aircraft, 131 = trains, 132/133 = cars. Fails if `dword_2040A2C` (override) is set. |

> **48 vs 50, settled.** The two have long been reported both ways round, and the IDB's model tables disagreed with these notes. The scripts decide it once `wm2field.tbl` is read alongside them: in `wmsetus.obj`, section 36 script 7 checks vehicle **48** and warps to entrance 64, whose `wm2field` record names field `bgsido_4` — inside Balamb Garden; script 11 checks vehicle **50** and warps to entrance 66 = `rgcock4`, the Ragnarok cockpit; script 10 boards on `FF18 50` and warps to `rgcock2`. So `Wm_ModelIdToVehicleCode` maps world model 1 to the **Ragnarok's** code and models 64/65 to **Balamb Garden's** — the opposite of how the `world_objslot_*` globals were named (now corrected in the community IDB). It follows that `world_docking_state` 5/6 are the Ragnarok active/approaching and 8/9 are the Garden's, as documented for `CHECK_WORLD_MAP_STATE`.
...
| ID   | Hex     | Name                       | Notes |
|------|---------|----------------------------|-------|
| −237 | `0xFF13` | `ADD_ENTITY`               | Register entity. `param1` = **world model class**, `param2` = **index into the Section 10 position records** (`World_BuildObjectInstanceList` at 0x544860 reads `spawn_positions + 16 × param2`); `0xFF` means the object places itself. Model classes are their own numbering, *not* the vehicle codes `CHECK_VEHICLE_TYPE` uses — `Wm_ModelIdToVehicleCode` (0x546F90) converts between them. Known: 0 = Squall, **1 = the Ragnarok**, 2/3 = the two big ships, **64/65 = the mobile Balamb Garden**, 70 = a train, 73–87 = cars and chocobos, 94 = the Jumbo Cactuar (spawned only while GF 13, Cactuar, is not owned). Classes 1 and 64/65 ignore `param2` and read their saved position instead. Note the cross-over with the `CHECK_VEHICLE_TYPE` codes: vehicle 50 (Ragnarok) is model 1, vehicle 48 (Garden) is model 64. Used only in Section 9. |
