...
In the PC executable `SCRIPT_CTURNR` (0x5273F0) and `SCRIPT_CTURNL` (0x5274C0) are byte-for-byte identical: both take the shortest rotational path, so the R/L (right/left) distinction has no effect. For a guaranteed turn direction use [CLOCKWISETURN](../16b-clockwiseturn/) / [COUNTERCLOCKWISETURN](../16c-counterclockwiseturn/) instead.

PC handler: `SCRIPT_CTURNR` at 0x5273F0.
