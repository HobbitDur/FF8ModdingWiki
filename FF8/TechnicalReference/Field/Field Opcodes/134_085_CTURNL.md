...
In the PC executable `SCRIPT_CTURNL` (0x5274C0) is byte-for-byte identical to `SCRIPT_CTURNR` (0x5273F0): both take the shortest rotational path, so the R/L (right/left) distinction has no effect. For a guaranteed turn direction use [CLOCKWISETURN](../16b-clockwiseturn/) / [COUNTERCLOCKWISETURN](../16c-counterclockwiseturn/) instead.

PC handler: `SCRIPT_CTURNL` at 0x5274C0.
