...
| 384–5411 | **savemap block** — verbatim copy of 5028 bytes from `SG_CHECKSUM` (0x1CFDC58) |
| 5412–8191 | zero padding |

* **Checksum**: CRC-16/CCITT (poly 0x1021, init 0xFFFF, final NOT; table-driven, `Save_ComputeChecksumCRC16CCITT` 0x500310) over savemap+80..+5023 (4944 bytes), stored at savemap+0 **and** duplicated at savemap+5024. No other obfuscation or encryption.
...
* **Load**: requires `CRC(buf+80, 4944)` to equal both stored copies; then 5028 bytes are copied to `SG_CHECKSUM`, the RNG is reseeded from save statistics, and `engine_restore_game_status` (0x495EF0) recomputes derived character/GF stats. No version or region checks beyond the magic.
* The PSX read-back verify states survive in the code but are **dead on PC**, and a leftover **debug quick-loader** (`Save_DebugLoadSaveFileByPath_UNUSED`, 0x4E2DE0) loads a raw file by path expecting magic 0x0FF8 with no CRC check.

## Preview block (savemap+0..127)

What the load menu shows, refreshed at save time by `Save_BuildSaveFileImage` (0x4E2EF0):
...
* **New game**: `Savemap_Clear` zeroes the savemap, then `Savemap_LoadNewGameDefaults_InitOut` (0x56DA10) loads the file **`init.out`** directly into savemap+80 — that file *is* the new-game savemap and can be edited to change starting inventory, stats, flags…
* **Playtime**: `Savemap_TickGameTimeAndCountdown` (0x4701B0), called from every module main loop, adds 2191 per frame to an accumulator; every 0x20000 overflow = +1 second (131072/2191 ≈ 59.82 Hz — the NTSC vsync rate, so playtime advances at PSX speed regardless of module frame rate). The script COUNTDOWN uses the same mechanism.
...
|----------|---------|-|----------|---------|
| `Save_CardScreen_Update` | 0x4E3090 | | `Save_EmuMC_OpenMapNameToPCPath` | 0x4C6BD0 |
| `Save_BuildSaveFileImage` | 0x4E2EF0 | | `Save_RecompressSaveFileLZSS` | 0x4C6E50 |
| `Save_ComputeChecksumCRC16CCITT` | 0x500310 | | `Archive_LZSSCompress` | 0x40FE1D |
| `Save_EmuMC_WriteFile` / `ReadFile` | 0x4C5500 / 0x4C5B00 | | `Savemap_Clear` / `LoadNewGameDefaults_InitOut` | 0x56D9F0 / 0x56DA10 |
| `Save_EmuMC_CreateFile` | 0x4C4FA0 | | `Savemap_TickGameTimeAndCountdown` | 0x4701B0 |
| `Save_DebugLoadSaveFileByPath_UNUSED` | 0x4E2DE0 | | `ChocoboWorld_CheckChocorpgGetId` | 0x4C63C0 |
