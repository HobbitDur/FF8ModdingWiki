...
## Segment streaming (`Wmx_UpdateSegmentStreaming`)
...
* **Alternate world states** — story flags in `wm_worldStateFlags` (Fisherman's Horizon bridge, the Lunar Cry crater, the shield around it, …) remap `fileSegIdx` through internal tables (`Wmx_ApplyAlternateSegmentRemap`) before the read, which is how replacement segments stored later in wmx.obj appear transparently.
...
`World_UpdateAndDrawTerrain_Fog` or `_NoFog` (selected once per frame by the `WORLDMAP_WITHOUT_FOG` registry bit) orchestrates:
...
|---|---|---|
| `Wmx_UpdateSegmentStreaming` | 0x5531F0 | Rebuilds the wanted segment set and drives loading |
| `Wmx_ApplyAlternateSegmentRemap` | 0x553980 | Remaps `fileSegIdx` for alternate world states |
| `Wmx_GetSquareBlockPtr` | 0x553E00 | Resolves a square's block pointer |
| `Wmx_FindSegmentNode` | 0x553960 | Looks up a segment's streaming node |
| `World_UpdateAndDrawTerrain_Fog` | 0x53BB90 | Per-frame terrain update/draw, fogged variant |
| `World_UpdateAndDrawTerrain_NoFog` | 0x53C750 | Per-frame terrain update/draw, fogless variant |
| `World_DrawNearBlock_Fog` / `_NoFog` | 0x552380 / 0x553FD0 | Near-square triangle emission |
| `World_DrawNearBlock_RagnarokFlight_Fog` / `_NoFog` | 0x5519B0 / 0x5543A0 | Near-square emission, Ragnarok-flight variant |
| `World_DrawFarTerrainBlock` | 0x5513A0 | Distant square rendering with LOD swap |
| `World_ResolveGroundProbes` | 0x53E2A0 | Resolves ground probes against square blocks |
| `World_PointInTriangleGetHeight` | 0x402620 | XZ point-in-triangle height test |
| `World_IsTriangleWalkable` | 0x53E6B0 | Tests the vehicle walkability mask of a triangle |
| `World_PretransformBlockVertices` | 0x4022D0 | Camera-relativizes and curvature-sinks vertices |
| `World_ProjectTerrainVertices` | 0x4023D0 | Projects terrain vertices to screen space |
| `World_SetupGroundProbeRow` | 0x53D8A0 | Places a row of ground probes |
| `World_CullVisibleSquareList` | 0x53D3C0 | Frustum-culls the visible square list |
| `wmx_segmentCache` (16 slots) | 0xC75D04 | Segment cache slots (global) |
| `wm_worldStateFlags` | 0x2036BB6 | Story/alternate-world-state flags (global) |
