...
* **Depth** — one Z ordering table per frame (0..4095, inside the double-buffered render context, drawn from 4095 down to 0): tiles insert at their `.map` Z, character polygons and shadows at their camera-space depth / 4 (GTE SZ >> 2) — so per-tile occlusion of characters costs nothing. A tile Z is therefore the depth of the object it shows / 4; base picture tiles use 4094. See [Z: draw depth]({{ site.baseurl }}/technical-reference/field/field-file-format/field-background-tile-data/#z-draw-depth).
