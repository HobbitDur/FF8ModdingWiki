---
title: Section 5 - Animation sequences
layout: default
parent: Monster files (c0mxxx.dat)
permalink: /technical-reference/battle/monster-files-c0mxxxdat/section-5-animation-sequences/
nav_order: 5
---
...
- **Camera sequences** ([section 6](../section-6-camera-sequence/)): same VM with camera-specific callbacks (see that section).
...
This queues the animation by the ID defined in [Section 3](../section-3-model-animation/) and pauses the sequence until the animation finishes
...
|--------------------|--------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 80 XX              | 1      | Step **dynamic texture animation** XX ([section 4](../section-4-dynamic-texture-data/)). If `number_destination` > 0: advances the frame counter (data byte 6 = speed in 1/8th frame units, byte 7 = counter) and blits the next dest UV. If `number_destination` == 0: scrolls the texture region in VRAM by byte 6 pixels (UV scroll, e.g. conveyor/water)                               |
...
| A6 X1 + 4×int16    | 9      | **Detach model part** (body parts falling/flying off, e.g. Gerogero). X1 = object index in [section 2](../section-2-model-geometry/) (the model part). The part detaches from the skeleton at its current bone position and flies off with initial velocity (vx, vy, vz) given by the first three int16; each frame the velocity decays by 1/8 (air drag) and +100/frame is added on Y (gravity), so the part follows a ballistic arc for the 15 frames the task runs. The 4th int16 is stored in the task but its use is unidentified. The part is simultaneously flagged in the hidden-parts bitmask (same bitmask as E5 0x20); opcode 81 restores it later |
...
| AB X1 X2 ... A1    | var    | **Renzokuken init** (character weapons only). Reads two bytes, then *pre-scans* the rest of the sequence up to the next A1 without executing it: for every animation opcode (&lt; 0x80) it looks up the animation's frame count in [section 3](../section-3-model-animation/) and accumulates it; each A0 closes the current bucket and starts a new one. The resulting list of per-stage durations (in frames) is what drives the R1-trigger timing bar — i.e. the timing windows are computed from the actual animation lengths of the slashes that follow. Execution then resumes normally at the opcodes that were scanned |
...
| B5 / B6 X1 X2 (X3) | var    | Play local (monster .dat [section 9](../section-9-sounds/) AKAO) sound X1. X2 = flag byte (see B8). X1 >= 7 unexpected                                                                                                                                                                                                                                       |
