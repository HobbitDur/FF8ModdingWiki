...
| 2 Counter / 3 Death | End-of-action processors (`processBattleActionCompletion` and two variants) | During damage application, each victim is stamped with a *pending reaction section* (battle-slot field `pending_ai_section` at offset 0xC7, enum `PendingAiSection`: 2 = was hit and survived → Counter, 3 = died with scripted death → Death; formerly named `value_is_2_when_targeted` before its role was understood) plus an `inAction` order number. When the attacker's action fully completes, the processor walks all stamped slots **in the order they were hit** and queues each one's Counter (if alive) or Death section. This deferred, ordered mechanism is why Counter always runs after the attack fully resolves. |
...
**Gating flags — now fully traced.** The two pre-counter dispatch sites use two *different* per-slot flags (verified from the raw disassembly of `setMonsterInfoFromDatInfoSection`, the .dat stat loader):

- **Killing hit** → requires `BATTLE_FLAG_DEATH_AI` (bit `0x20` of `flag_data`). Set at monster load, and the rule is byte-exact: the loader reads **the very first bytecode byte of the Death section** (`ai_subsection + death_offset`) and sets the flag iff that byte is not `0x00` (the `stop`/`return` opcode). Only this first byte matters — a death section whose first opcode is `stop` is treated as empty even if bytes follow, and the *content of the pre-counter section plays no role at all*.
- **Surviving hit** → requires bit `0x10` of `flag_data`, which the same loader sets **unconditionally for every monster** (initial monster `flag_data` = `0x11`; characters get `0x8801`, without it). Since it stays set for as long as the monster is loaded, this gate is effectively always open — pre-counter runs on surviving hits for **every** monster, regardless of its death section.

Together these two flags explain the long-standing empirical note precisely: "pre-counter only runs after a *killing* attack if the death section has code in it" — because on a kill the dispatch goes through the scripted-death branch (gated by `0x20`), while on ordinary hits it is unconditional. Consequences for modders: giving a monster pre-counter code always works for on-hit reactions, but its on-death pre-counter (and its whole Death script) is silently disabled unless the Death section starts with a real opcode.

**On a surviving hit** (monster took damage but lives): pre-counter runs for every monster (gate `0x10`, always set), provided the hit came from a real attack (not a system effect), and only when no further hits of the same action are pending — i.e. for a multi-hit attack it runs once, on the final hit. The target is then stamped with pending-reaction = 2 so its Counter section gets queued when the action completes.

**On a killing hit** (monster with `BATTLE_FLAG_DEATH_AI`, i.e. a Death section starting with a real opcode): pre-counter runs, and then the engine **removes the Death status it just applied**. The monster is technically alive again; it is stamped with pending-reaction = 3 so its Death section gets queued instead. Death is thereby *deferred to the script*: the queued Death section must eventually execute the **_die_** opcode (or the monster fights on at 0 HP — the classic softlock documented in [Battle Scripts](../battle-scripts/)). Monsters *without* the flag skip all of this and die immediately with the standard death handling (drops, cards, AP, reaction animation 3) — and their pre-counter does **not** run for that killing hit.
...
| `setMonsterInfoFromDatInfoSection` | `0x48BBD0` | Monster stat loader (.dat section 7 → battle slot); sets initial `flag_data` (0x11) and `BATTLE_FLAG_DEATH_AI` (0x20) from the Death section's first byte |
| `clearBattleSlotData` | `0x48C5C0` | Resets a battle slot (flags 0, Death status, timers disabled) before an incoming monster is loaded into it |
