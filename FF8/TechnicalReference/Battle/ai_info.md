...
| 0x22 (34) | **printWithDelay** *(renamed from `printAlt`)* | 2 | Displays battle text `{param1}` **after a delay** of `{param2}` text-queue ticks, where **_print_** (0x01) shows it at once. It registers a text-queue task whose callback counts `param2` down each tick and queues the caption (speed 0x1E, priority 0xF0, vs 0x80 for **_print_**) when it reaches 0. The delay is a wait before showing, not a display duration. Not used by any vanilla monster script (all 200 `c0m` files scanned) | **Renamed 2026-09.** Both parameter bytes are read by the game (text id, then delay) |
...
| 0x2D (45) | elemDmgMod | 3 | **Verified 2026-09.** Writes the monster's own elemental resistance at runtime. Params: `elem_index` (1 byte), then a 16-bit little-endian value. Effective address is `BATTLE_SLOT_DATA + 0x44 + 208*slot + 2*elem_index`, i.e. `elem_def[elem_index]` in [Battle Slot Data](../battle-slot-data/) — not a generic local-var slot. Readers apply `800 - v` (physical) or `900 - v` (magic), which matches IfritAI encoding an authored percentage as `900 - percent`. **No bounds check on `elem_index`**: it is a raw 0-255 bytecode byte, so the opcode can write a word anywhere from `+0x44` to `+0x242` from the slot base — over `timer`, coordinates, `flag_data`, `status_1`, `mental_res` and into following slots (index 8 lands in `timer[0]`) | See [Element System](../element-system/) |
