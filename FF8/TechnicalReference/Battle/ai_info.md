...
Note: **pre-counter** code will **ONLY** be executed after an attack that kills a monster if the monster's **death** section has code in it (apart from "return"). *Mechanism:* a single per-monster engine flag gates both scripted-death handling and pre-counter execution, so the two always come together.  
